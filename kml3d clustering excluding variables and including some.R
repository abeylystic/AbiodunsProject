...
# drop total GDP
data_no_gdp <- subset(data, select = -c(All))


complete_combinations <- expand.grid(FIPS_y = unique(data_no_gdp$FIPS_y), 
                                     TimePeriod = unique(data_no_gdp$TimePeriod))
...
final_data <- new_data %>%
  left_join(data_no_gdp, by = c("FIPS_y", "TimePeriod"))
...
# Export the data
write.csv(df_cl2, file='kml3d_no_gdp.csv', row.names=FALSE)
...
# Export the data
write.csv(df_cl2, file='kml3d_no_gdp_m4.csv', row.names=FALSE)
...
# Export the data
write.csv(df_cl2, file='kml3d_no_m4.csv', row.names=FALSE)
